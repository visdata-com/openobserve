...
pub async fn extract_patterns(
    ...
    // Visdata feature implementation (when enterprise is not enabled)
    #[cfg(all(feature = "visdata", not(feature = "enterprise")))]
    {
        use tokio::sync::mpsc;
        ...
        log::info!(
            "[PATTERNS trace_id {}] Pattern extraction requested for {}/{} (visdata)",
            trace_id,
            org_id,
            stream_name
        );
        ...
        // Create audit context (simplified for visdata)
        let audit_ctx = Some(crate::common::meta::search::AuditContext {
            method: in_req.method().to_string(),
            path: in_req.path().to_string(),
            query_params: in_req.query_string().to_string(),
        ...
        // Set size limit for pattern extraction using visdata config
        if req.query.size == 0 || req.query.size == -1 {
            let visdata_cfg = visdata::config::get_config().await;
            req.query.size = if visdata_cfg.log_patterns_max_logs > 0 {
                visdata_cfg.log_patterns_max_logs as i64
            } else {
                config::get_config().limit.query_default_limit
            };
        }
        ...
        log::info!(
            "[PATTERNS trace_id {}] Query configuration (visdata) - size: {}, sampling_ratio: {:?}",
            trace_id,
            req.query.size,
            req.query.sampling_ratio
        );
        ...
        while let Some(result) = rx.recv().await {
            ...
            match result {
                Ok(config::meta::search::StreamResponses::PatternExtractionResult { patterns }) => {
                    log::info!(
                        "[PATTERNS trace_id {}] Received pattern extraction result (visdata)",
                        trace_id
                    );
                    patterns_result = Some(patterns);
                    break; // We only need the patterns
        ...
        if let Some(patterns) = patterns_result {
            log::info!(
                "[PATTERNS trace_id {}] Successfully extracted patterns (visdata)",
                trace_id
            );
            Ok(MetaHttpResponse::json(patterns))
        } else {
            log::warn!(
                "[PATTERNS trace_id {}] No patterns were extracted (visdata)",
                trace_id
            );
            Ok(MetaHttpResponse::json(serde_json::json!({
                "patterns": [],
    ...
    // No enterprise or visdata feature
